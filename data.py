...
class myAugmentation(object):
    ...
    def __init__(self, train_path="./deform/train", label_path="./deform/label", merge_path="./deform/merge",
                 aug_merge_path="./deform/aug_merge", aug_train_path="./deform/aug_train",
                 aug_label_path="./deform/aug_label", img_type="png"):
    ...
    def Augmentation(self):
        ...
        for i in range(len(trains)):
            ...
            # 使用opencv保存
            # cv2.imwrite(path_merge + "/" + str(i) + "." + imgtype, img_tmp)
    ...
    def splitMerge(self):
        ...
        for i in range(self.slices):
            ...
            for imgname in train_imgs:         # rindex("/") 是返回'/'在字符串中最后一次出现的索引
                midname = imgname[imgname.rindex("/") + 3:imgname.rindex("." + self.img_type)]# 获得文件名(不包含后缀)
...
class dataProcess(object):
    ...
    def create_train_data(self):
        ...
        for indir in os.listdir(self.aug_merge_path):
            ...
            imgs = glob.glob(trainPath + '/*' + '.png')
            for imgname in imgs:
                trainmidname = imgname[imgname.rindex('/') + 13:]
                # labelimgname = imgname[imgname.rindex('/') + 1:imgname.rindex('_')] + '_label.tif'
                labelimgname = imgname[imgname.rindex('/') + 13:imgname.rindex('_')] + '_label.png'
                print(trainmidname, labelimgname)
                strss = trainPath + '/' + trainmidname
    ...
    def create_test_data(self):
        ...
        for imgname in imgs:
            midname = imgname[imgname.rindex("/") + 6:]   # 图像的名字
